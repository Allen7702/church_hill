...
{{--            </tfoot>--}}
            ...
            @foreach($enrollements as $item)
                <tr>
                    <td>{{$loop->index+1}}</td>
                    <td><span class="badge badge-circle badge-{{$item->status == 'fresher'?'info':'success'}}">{{$item->status == 'fresher'? 'wanasoma': 'wamemaliza'}}</span></td>
                    <td>{{$item->total}}</td>
                    <td>{{$item->year}}</td>
                    <td>
                        <a href="{{route('mafundisho_enrollments.index', ['year' => $item->year, 'type' => $item->type, 'status' => $item->status])}}"><button class="btn btn-sm btn-info">Angalia</button></a>
                    </td>
                </tr>
            @endforeach
            </tbody>
...
<script type="text/javascript">
    ...
    Highcharts.chart('sadaka_chart', {
        ...
        xAxis: {
            categories: [
                <?php
                    foreach(json_decode($enrollements,true) ?? '' as $mwaka){
                        echo $mwaka['total'];
                        echo ',';
                    }
                ?>
        ...
        series: [{
            ...
            data: [
                <?php
                    foreach(json_decode($enrollements,true) ?? '' as $kiasi){
                        echo $kiasi['year'];
                        echo ',';
                    }
                ?>
