    ...
    <div class="card">
        <div class="card-header">
            <div class="row">
                ...
                <div class="col-md-6 text-right">
                    <button id="tukioBtn" class="btn btn-sm btn-info">Ongeza tukio</button>
                </div>
            </div>
        </div>
        ...
        <div class="card-body">
            <table class="table w-100" id="table">
                ...
                <tbody>
                    @foreach ($data_matukio as $item)
                    <tr>
                        <td>{{$loop->index+1}}</td>
                        <td><img src="{{url('uploads/images/'.$item->picha)}}" class="img" width="60px"></td>
                        <td title="{{$item->kichwa}}">{{Str::limit($item->kichwa,'12','...')}}</td>
                        <td>{{$item->uchapishaji}}</td>
                        <td title="{{$item->maelezo}}">{{Str::limit($item->maelezo,'15','..')}}</td>
                ...
                <form id="tukioForm" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="">Kichwa cha tukio:</label>
                            <input class="form-control" type="text" id="kichwa" name="kichwa" placeholder="Kichwa cha tukio" required>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="maelezo" class="form-control" id="maelezo" placeholder="Maelezo kuhusu tukio" rows="2" required></textarea>
                        </div>
...
<script>
    $("document").ready(function(){
        ...
        $('#tukioBtn').on('click', function(){
            $('#tukioForm')[0].reset();
            $('.modal-title').text('Ongeza tukio');
            $('#action').val('Generate');
        ...
        $('#tukioForm').on('submit', function(event){
            ...
            var data_url = "{{route('matukio.store')}}";

            if($('#action').val() == "Update")
            var data_url = "{{route('matukio.update')}}";
        ...
        $(document).on('click','.view', function(event){
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    $('.modal-title').text('Angalia taarifa za matukio');
                    $('#tukioModal').modal({backdrop: 'static',keyboard: false});
                    $('#tukioModal').modal('show');
                }
            })
        ...
        $(document).on('click','.edit', function(event){
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    $('#uchapishaji').val(data.result.uchapishaji);
                    $('#maelezo').val(data.result.maelezo);
                    $('.modal-title').text('Badili taarifa za matukio');
                    $('#tukioModal').modal({backdrop: 'static',keyboard: false});
                    $('#tukioModal').modal('show');
                }
            })
