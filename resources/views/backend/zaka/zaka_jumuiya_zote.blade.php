        ...
        <div class="card-header">
            <div class="row">
                ...
                <div class="col-md-6 col-sm-12">
                    <h4 class="text-bold">UTOAJI WA ZAKA JUMUIYA ZOTE</h4>
                </div>
                ...
                <div class="col-md-6 text-right">
                    ...
                    <button class="btn btn-info btn-sm mr-1">PDF</button>
                    <button class="btn btn-info btn-sm mr-1">Excel</button>
                </div>
        ...
        <div class="card-body">
            <table class="table_other table w-100" id="table">
                ...
                <tfoot>
                    ...
                    <th colspan="2">{{number_format($zaka_total,2)}}</th>
                </tfoot>
                <tbody>
                    @foreach ($data_zaka as $item)
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="{{url('zaka_risiti',['id'=>$item->id])}}"><button class="btn bg-info btn-sm mr-1"><i class="fas fa-fw fa-print text-white"></i></button></a>
...
$("document").ready(function(){
        ...
        $('#zakaForm').on('submit', function(event){
            ...
            var data_url = "{{route('zaka.update')}}";
        ...
        $(document).on('click','.edit', function(event){
            ...
            $.ajax({
                ...
                success: function(data){
                    $('#submitBtn').attr('disabled',false);
                    $('#createMwanajumuiyaDiv').hide();
                    $('#updateMwanajumuiyaDiv').show();
                    $('#createJumuiyaDiv').hide();
                    $('#updateJumuiyaDiv').show();
                    $('#submitBtn').show();
                    $('#submitBtn').html('Badilisha');
                    $('#action').val('Update');
                    $('#hidden_id').val(id);
                    $('#mwanajumuiya_update').val(data.result.mwanajumuiya);
                    $('#jumuiya_update').val(data.result.jumuiya);
                    $('#tarehe').val(data.result.tarehe);
                    $('#kiasi').val(data.result.kiasi);
                    $('.modal-title').text('Badili taarifa za zaka');
                    $('#zakaModal').modal({backdrop: 'static',keyboard: false});
                    $('#zakaModal').modal('show');
                }
            })
