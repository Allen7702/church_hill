    ...
    <div class="card-header">
        <div class="row">
            <div class="col-md-6 col-sm-12">
                <h4 class="text-bold">MAKISIO BAJETI YA MATUMIZI</h4>
            </div>
                    ...
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="aina_ya_matumizi" id="aina_ya_matumizi" class="form-control">
                                    <option value="">Chagua matumizi</option>
                                    @foreach ($data_huduma as $item)
                                        <option value="{{$item->aina_ya_huduma}}">{{$item->aina_ya_huduma}}</option>
                                    @endforeach
                                    @foreach ($data_matumizi as $item)
                                        <option value="{{$item->aina_ya_matumizi}}">{{$item->aina_ya_matumizi}}</option>
                                    @endforeach
                                </select>
                            </div>
...
<script type="text/javascript">
    $("document").ready(function(){
        ...
        $('#bajetiForm').on('submit', function(event){
            ...
            var data_url = "{{route('bajeti_makisio_matumizi.store')}}";

            if($('#action').val() == "Update")
            var data_url = "{{route('bajeti_makisio_matumizi.update')}}";
        ...
        $(document).on('click','.edit', function(event){
            ...
            $.ajax({
                url: "/bajeti_makisio_matumizi/" + id + "/edit",
                dataType: "JSON",
                success: function(data){
                    $('#createDiv').hide();
                    $('#updateDiv').show();
        ...
        $(document).on('click', '.delete', function (event) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "/bajeti_makisio_matumizi/destroy/" + delete_id,
                        success: function (data) {
