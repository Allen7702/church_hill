    ...
    <div class="card-header">
        <div class="row">
            <div class="col-md-6 col-sm-12">
                <h4 class="text-bold">AINA ZA MISA</h4>
            </div>
            <div class="col-md-6 text-right">
                <button id="misaBtn" class="btn btn-info btn-sm mr-2">Ongeza </button>
                <a href="{{url('aina_za_misa_pdf')}}"><button class="btn btn-info btn-sm mr-2">PDF</button></a>
                <a href="{{url('aina_za_misa_excel')}}"><button class="btn btn-info btn-sm mr-0">Excel</button></a>
            </div>
        </div>
    </div>
    <div class="card-body">
...
<div id="misaModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="my-modal-title" aria-hidden="true">
            ...
            <form id="misaForm">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="">Jina la misa:</label>
                        <input class="form-control" type="text" id="jina_la_misa" name="jina_la_misa" placeholder="Jina la misa" required>
                    </div>
...
<script type="text/javascript">
    $("document").ready(function(){
        ...
        $('#misaBtn').on('click', function(){
            //preventing resubmission of data
            $('#submitBtn').attr('disabled',false);
            $('#misaForm')[0].reset();
            $('.modal-title').text('Ongeza misa');
            $('#action').val('Generate');
            $('#submitBtn').show();
            $('#submitBtn').html('Wasilisha');
            $('#misaModal').modal({backdrop: 'static',keyboard: false});
            $('#misaModal').modal('show');
        })

        //submitting values
        $('#misaForm').on('submit', function(event){
            ...
            var data_url = "{{route('aina_za_misa.store')}}";

            if($('#action').val() == "Update")
            var data_url = "{{route('aina_za_misa.update')}}";
        ...
        $(document).on('click','.view', function(event){
            ...
            $.ajax({
                url: "/aina_za_misa/" + id + "/edit",
                dataType: "JSON",
                success: function(data){
                    $('#submitBtn').hide();
                    $('#hidden_id').val(id);
                    $('#jina_la_misa').val(data.result.jina_la_misa);
                    $('#maelezo').val(data.result.maelezo);
                    $('.modal-title').text('Angalia taarifa za misa');
                    $('#misaModal').modal({backdrop: 'static',keyboard: false});
                    $('#misaModal').modal('show');
                }
            })
        ...
        $(document).on('click','.edit', function(event){
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    $('#jina_la_misa').val(data.result.jina_la_misa);
                    $('#maelezo').val(data.result.maelezo);
                    $('.modal-title').text('Badili taarifa za misa');
                    $('#misaModal').modal({backdrop: 'static',keyboard: false});
                    $('#misaModal').modal('show');
                }
            })
        ...
        $(document).on('click', '.delete', function (event) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "/aina_za_misa/destroy/" + delete_id,
                        success: function (data) {
