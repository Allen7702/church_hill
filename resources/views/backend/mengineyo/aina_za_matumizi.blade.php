    ...
    <div class="card-header">
        <div class="row">
            <div class="col-md-6 col-sm-12">
                <h4 class="text-bold">AINA ZA MATUMIZI</h4>
            </div>
            <div class="col-md-6 text-right">
                <button id="matumiziBtn" class="btn btn-info btn-sm mr-2">Ongeza aina</button>
...
<div id="matumiziModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="my-modal-title" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form id="matumiziForm">
                @csrf
                <div class="modal-body">

                    <div class="form-group">
                        <label for="">Aina ya matumizi:</label>
                        <input class="form-control" type="text" id="aina_ya_matumizi" name="aina_ya_matumizi" placeholder="Aina ya matumizi" required>
                    </div>
...
<script type="text/javascript">
    $("document").ready(function(){
        ...
        $('#matumiziBtn').on('click', function(){
            $('#submitBtn').attr('disabled',false);
            $('#matumiziForm')[0].reset();
            $('.modal-title').text('Sajili aina ya matumizi');
            $('#action').val('Generate');
            $('#submitBtn').html('Wasilisha');
            $('#submitBtn').show();
            $('#matumiziModal').modal({backdrop: 'static',keyboard: false});
            $('#matumiziModal').modal('show');
        })

        //submitting values
        $('#matumiziForm').on('submit', function(event){
            ...
            var data_url = "{{route('aina_za_matumizi.store')}}";

            if($('#action').val() == "Update")
            var data_url = "{{route('aina_za_matumizi.update')}}";
        ...
        $(document).on('click','.view', function(event){
            ...
            $.ajax({
                url: "/aina_za_matumizi/" + id + "/edit",
                dataType: "JSON",
                success: function(data){

                    $('#submitBtn').hide();
                    $('#hidden_id').val(id);
                    $('#aina_ya_matumizi').val(data.result.aina_ya_matumizi);
                    $('#maelezo').val(data.result.maelezo);
                    $('.modal-title').text('Angalia aina ya matumizi');
                    $('#matumiziModal').modal({backdrop: 'static',keyboard: false});
                    $('#matumiziModal').modal('show');
                }
            })
        ...
        $(document).on('click','.edit', function(event){
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    $('#aina_ya_matumizi').val(data.result.aina_ya_matumizi);
                    $('#maelezo').val(data.result.maelezo);
                    $('.modal-title').text('Badili aina ya matumizi');
                    $('#matumiziModal').modal({backdrop: 'static',keyboard: false});
                    $('#matumiziModal').modal('show');
                }
            })
        ...
        $(document).on('click', '.delete', function (event) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "/aina_za_matumizi/destroy/" + delete_id,
                        success: function (data) {
