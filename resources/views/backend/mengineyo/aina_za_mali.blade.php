    ...
    <div class="card-header">
        <div class="row">
            <div class="col-md-6 col-sm-12">
                <h4 class="text-bold">Aina za mali</h4>
            </div>
            <div class="col-md-6 text-right">
                <button id="ainaBtn" class="btn btn-info btn-sm mr-2">Ongeza</button>
                <a href="{{url('aina_za_mali_pdf')}}"><button class="btn btn-info btn-sm mr-2">PDF</button></a>
                <a href="{{url('aina_za_mali_excel')}}"><button class="btn btn-info btn-sm mr-0">Excel</button></a>
            </div>
        </div>
    </div>
    <div class="card-body">
...
<div id="ainaModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="my-modal-title" aria-hidden="true">
            ...
            <form id="ainaForm">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" id="aina_ya_mali" name="aina_ya_mali" placeholder="Aina ya mali" required>
                    </div>

                    <div class="form-group">
                        <label for="">Maelezo:</label>
                        <textarea name="maelezo" class="form-control" id="maelezo" placeholder="Maelezo kuhusu aina ya mali" rows="2"></textarea>
                    </div>
...
<script type="text/javascript">
    $("document").ready(function(){
        ...
        $('#ainaForm').on('submit', function(event){
            ...
            var data_url = "{{route('aina_za_mali.store')}}";

            if($('#action').val() == "Update")
            var data_url = "{{route('aina_za_mali.update')}}";
        ...
        $(document).on('click','.view', function(event){
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    $('.modal-title').text('Angalia aina ya mali');
                    $('#ainaModal').modal({backdrop: 'static',keyboard: false});
                    $('#ainaModal').modal('show');
                }
            })
        ...
        $(document).on('click','.edit', function(event){
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    $('#aina_ya_mali').val(data.result.aina_ya_mali);
                    $('#maelezo').val(data.result.maelezo);
                    $('.modal-title').text('Badili aina ya mali');
                    $('#ainaModal').modal({backdrop: 'static',keyboard: false});
                    $('#ainaModal').modal('show');
                }
            })
        ...
        $(document).on('click', '.delete', function (event) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "/aina_za_mali/destroy/" + delete_id,
                        success: function (data) {
