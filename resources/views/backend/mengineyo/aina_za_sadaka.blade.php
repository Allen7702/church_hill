    ...
    <div class="card-header">
        <div class="row">
            <div class="col-md-6 col-sm-12">
                <h4 class="text-bold">AINA ZA SADAKA</h4>
            </div>
            ...
            <form id="sadakaForm">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="">Jina la sadaka:</label>
                        <input class="form-control" type="text" id="jina_la_sadaka" name="jina_la_sadaka" placeholder="Jina la sadaka" required>
                    </div>
...
<script type="text/javascript">
    $("document").ready(function(){
        ...
        $('#sadakaBtn').on('click', function(){
            ...
            $('#sadakaForm')[0].reset();
            $('.modal-title').text('Ongeza aina ya sadaka');
            $('#action').val('Generate');
            $('#submitBtn').show();
        ...
        $('#sadakaForm').on('submit', function(event){
            ...
            var data_url = "{{route('aina_za_sadaka.store')}}";

            if($('#action').val() == "Update")
            var data_url = "{{route('aina_za_sadaka.update')}}";
        ...
        $(document).on('click','.view', function(event){
            ...
            $.ajax({
                url: "/aina_za_sadaka/" + id + "/edit",
                dataType: "JSON",
                success: function(data){
                    $('#submitBtn').hide();
                    $('#hidden_id').val(id);
                    $('#jina_la_misa').val(data.result.jina_la_sadaka);
        ...
        $(document).on('click','.edit', function(event){
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    $('#action').val('Update');
                    $('#hidden_id').val(id);
                    $('#jina_la_sadaka').val(data.result.jina_la_sadaka);
                    $('#maelezo').val(data.result.maelezo);
        ...
        $(document).on('click', '.delete', function (event) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "/aina_za_sadaka/destroy/" + delete_id,
                        success: function (data) {
