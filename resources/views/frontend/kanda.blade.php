...
<section id="hero">
    <div class="hero-container d-flex flex-row">
        <div class="w-50 py-9 px-3">
            ...
            <h2 class="animate__animated animate__fadeInDown text-black-50" align="center">@if($sahihisha_kanda->name) {{$sahihisha_kanda->name}}@else Kanda @endif</h2>
            <p class="animate__animated animate__fadeInUp text-black-50  w-100" align="center">Karibu katika wavuti ya kanisa, usisite kuangalia @if($sahihisha_kanda->name) {{$sahihisha_kanda->name}} @else Kanda @endif mbalimbali.</p>
        </div>
...
<section id="featured" class="featured">
    <div class="container">
        ...
        <div class="section-title" data-aos="fade-up">
            <h2>@if($sahihisha_kanda->name) {{$sahihisha_kanda->name}} @else Kanda @endif Zetu</h2>
        </div>

        @if($data_kanda->isNotEmpty())


            <div class="row">
                @foreach ($data_kanda as $row)
                <div class="col-lg-4 mb-3">
                    <div class="icon-box">
                        <h3><a href="">{{$row->jina_la_kanda}}</a></h3><hr>
                        <p>{{$row->comment}}</p>
                    </div>
                </div>
                @endforeach
